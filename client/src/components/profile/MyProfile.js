...
export default function MyProfile() {
    ...
    const singleFileUploadHandler = async (event) => {
        ...
        if (selectedFile) {
            ...
            await Axios.post(`${url}/images/profile-img-upload`, data, {
                headers: {
        ...
        <div className="profile-container">
            ...
            <div className="card-body d-flex justify-content-center flex-column">
                <div className="my-profile-name">
                    {userData.lastName && <h2>{`${userData && userData.name.charAt(0).toUpperCase()}${userData && userData.name.slice(1)} ${userData && userData.lastName.charAt(0).toUpperCase()}${userData && userData.lastName.slice(1)}`}</h2>}
                </div>
